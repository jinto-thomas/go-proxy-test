...
func updateChartDbNSE(ch <-chan JsonQuote) {
...
func updateChartDbNFO(ch <-chan JsonQuote) {
	buf := make([]JsonQuote, 200)
	mutex := new(sync.Mutex)
...
func updateValues(buf *[]JsonQuote, db *sql.DB, exc string) {
	...
	if strings.Compare(exc, "NSE") == 0 {
		query1 = "insert ignore into nse_chart (trad_sym, symbol, ltp, open, close, high, low, volume, updated_at) values "
	} else {
...
func main() {
	...
	nsequoteQueueChannel := make(chan JsonQuote, 20)
	nfoquoteQueueChannel := make(chan JsonQuote, 20)

	var wg sync.WaitGroup
	wg.Add(1)
	go updateChartDbNSE(nsequoteQueueChannel)
	go updateChartDbNFO(nfoquoteQueueChannel)
